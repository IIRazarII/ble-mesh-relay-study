...
%
% Every simulation parameter is exposed as a name-value argument, so no
% constant has to be edited inside the file. Calling the function with no
% arguments reproduces "Experiment D" (Grid Topology): 20 relay nodes in a
% 5x4 grid with 8 m spacing, two source-destination pairs at the edges, a
% 9 m coverage range, 400 messages per source at 1 packet/s, two network
% transmissions, TTL 127, a 10 ms Scan Interval and Stateful Preemption.
%
...
%
%   % Reduced run
%   results = runPreemptionSimulation(RelayStrategy = 2, ScanInterval = 100e-3, ...
%                                     PacketsPerSource = 5);
%
%   % Reduced run with logs, to verify the mechanism
%   results = runPreemptionSimulation(RelayStrategy = 2, ScanInterval = 100e-3, ...
%                                     PacketsPerSource = 5, EnablePreemptionLog = true, EnableAdvEventLog = true);
%
%   % Single burst: 100 messages in 10 ms, then 10 s to drain the network
%   results = runPreemptionSimulation(PacketsPerSource = 100, BurstSize = 100, ...
%                                     TrafficOnTime = 10e-3, PacketRate = 0.05, ...
%                                     SimTime = 10);
